...
export const updateVehicleTrip = async (req, res) => {
  ...
  const { endStation, actualDestination, cost, carbonSaved, rating, status } = req.body

  if (!endStation || !actualDestination || cost === undefined || carbonSaved === undefined || !status) {
  ...
  try {
    ...
    // 1. Actualizar el registro en la tabla Viaje_alquiler
    const result = await query(
      `UPDATE Viaje_alquiler
       ...
       RETURNING *`,
      [actualDestination, cost, carbonSaved, rating, status, tripId],
    )
    ...
    // 2. Obtener el id_vehiculo_individual del Viaje_alquiler para actualizar su estado
    const vehicleTripResult = await query(
      `SELECT id_vehiculo_individual FROM Viaje_alquiler WHERE id_viaje_alquiler = $1`,
      [tripId],
    )
    const vehicleId = vehicleTripResult.rows[0]?.id_vehiculo_individual

    if (vehicleId) {
      // 3. Actualizar el estado del vehículo a 'Disponible' y su estación
...
export const createPublicTransportTrip = async (req, res) => {
  ...
  try {
    ...
    const tripResult = await query(
      `INSERT INTO Viaje_normal (id_usuario, id_vehiculo_colectivo, id_parada_inicio, id_parada_fin, origen, destino, estado, current_station_index, full_route_snapshot)
...
export const updatePublicTransportTrip = async (req, res) => {
  const { tripId } = req.params
  const { actualDestination, status, cost, carbonSaved, rating } = req.body

  if (!actualDestination || !status || cost === undefined || carbonSaved === undefined || rating === undefined) {
    return res
      .status(400)
  ...
  try {
    const result = await query(
      `SELECT
        ...
        vn.full_route_snapshot AS full_route,
        vn.estado AS status,
        vn.costo,
...
export const updatePublicTransportTrip = async (req, res) => {
  const { tripId } = req.params
  const { actualDestination, status, rating } = req.body // Ya no recibimos cost ni carbonSaved del frontend
  ...
  try {
    ...
    // 1. Obtener los detalles del viaje y del vehículo colectivo asociado
    ...
    const { fecha_hora_inicio, precio_por_viaje, factor_huella_de_carbono } = tripDetailsResult.rows[0]
    ...
    const calculatedCost = Number.parseFloat(precio_por_viaje).toFixed(2) // Costo fijo por viaje
    const calculatedCarbonSaved = (durationInMinutes * Number.parseFloat(factor_huella_de_carbono)).toFixed(2) // Carbono ahorrado basado en duración y factor
    ...
    const result = await query(
      `UPDATE Viaje_normal
       ...
       WHERE id_viaje_normal = $6
       RETURNING *`,
      [actualDestination, status, calculatedCost, calculatedCarbonSaved, rating, tripId],
    )
...
export const updateVehicleTrip = async (req, res) => {
  const { tripId } = req.params
  const { endStation, actualDestination, rating, status } = req.body // Ya no recibimos cost ni carbonSaved del frontend
