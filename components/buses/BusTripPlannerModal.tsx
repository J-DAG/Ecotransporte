...
import { Bus, Train, Clock, ArrowRight, Navigation, CheckCircle, Star } from "lucide-react" // Importar Star
...
interface BusTripPlannerModalProps {
  isOpen: boolean
  onClose: () => void
  route: PublicTransportRoute | null
}

export default function BusTripPlannerModal({ isOpen, onClose, route }: BusTripPlannerModalProps) {
  const { currentPublicTransportTrip, startPublicTransportTrip, getOffPublicTransport } = useApp()
  const [selectedStartStation, setSelectedStartStation] = useState<string>("")
  const [selectedDestination, setSelectedDestination] = useState<string>("")
  const [rating, setRating] = useState<number>(5) // Estado para la calificación

  const handleStartTrip = () => {
    if (route && selectedStartStation && selectedDestination) {
      startPublicTransportTrip(route.id, selectedStartStation, selectedDestination)
  ...
  const handleGetOff = (stationName: string) => {
    getOffPublicTransport(stationName, rating) // Enviar la calificación seleccionada
  ...
  // Vista normal de planificación de viaje
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2">
            ...
            <span>Planificar Viaje - {route?.name}</span>
          </DialogTitle>
          <DialogDescription>Selecciona tu parada de inicio y destino</DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-gray-50">
              ...
              <CardContent className="pt-0">
                <p className="text-sm text-gray-600">{route.fullRoute.length} estaciones en total</p>
              </CardContent>
            </Card>

            {/* Selección de parada de inicio */}
            <div>
              <h3 className="text-lg font-semibold mb-3">Parada de Inicio</h3>
              <Select
                value={selectedStartStation}
                onValueChange={(value) => {
                  setSelectedStartStation(value)
                  // Resetear el destino si la parada de inicio cambia
                  setSelectedDestination("")
                }}
              >
                <SelectTrigger>
                  <SelectValue placeholder="¿Dónde vas a abordar?" />
                </SelectTrigger>
                <SelectContent>
                  {/* Filtra para que la última parada no pueda ser seleccionada como inicio */}
                  {route.fullRoute
                    .filter((_, index) => index < route.fullRoute.length - 1)
                    .map((station) => (
            ...
            {/* Selección de parada de destino */}
            <div>
              <h3 className="text-lg font-semibold mb-3">Parada de Destino</h3>
              <Select
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="¿Dónde vas a bajar?" />
                </SelectTrigger>
                <SelectContent>
                  {selectedStartStation &&
                    route.fullRoute
                      .slice(route.fullRoute.indexOf(selectedStartStation) + 1) // Filtra paradas después de la de inicio
                      .map((station) => (
                        <SelectItem key={station} value={station}>
                          {station}
                        </SelectItem>
            ...
            {/* Resumen del viaje */}
            {selectedStartStation && selectedDestination && (
              <Card className="bg-green-50 border-green-200">
                <CardHeader>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span className="font-medium">Parada de inicio:</span>
                    <span>{selectedStartStation}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">Parada de destino:</span>
                    <span>{selectedDestination}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">Próximo {route.type === "bus" ? "bus" : "tranvía"}:</span>
                    <span className="text-green-700 font-semibold">{route.estimatedArrival}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium">Estaciones a recorrer:</span>
                    <span>
                      {Math.abs(
                        route.fullRoute.indexOf(selectedDestination) - route.fullRoute.indexOf(selectedStartStation),
                      )}{" "}
                      paradas
                    </span>
                  </div>
            ...
            <div className="flex space-x-3 pt-4">
              ...
              <Button
                ...
                disabled={!selectedStartStation || !selectedDestination}
                className="flex-1"
              >
                Iniciar Viaje
              </Button>
