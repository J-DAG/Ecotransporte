...
interface AppContextType {
  ...
  getOffPublicTransport: (stationName: string) => Promise<void>
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      if (vehicleResponse.ok) {
        setTrips(
          vehicleData.trips.map((trip: any) => ({
            ...
            cost: trip.cost !== null ? Number.parseFloat(trip.cost) : undefined,
            carbonSaved: trip.carbon_saved !== null ? Number.parseFloat(trip.carbon_saved) : undefined,
      ...
      if (currentTripResponse.ok && currentTripData.currentTrip) {
        if (currentTripData.type === "vehicle") {
          setCurrentTrip({
            ...
            cost:
              currentTripData.currentTrip.cost !== null
                ? Number.parseFloat(currentTripData.currentTrip.cost)
                : undefined,
            carbonSaved:
              currentTripData.currentTrip.carbon_saved !== null
                ? Number.parseFloat(currentTripData.currentTrip.carbon_saved)
                : undefined,
            rating:
  ...
  const endVehicleTrip = async (endStation: string, rating: number) => {
    ...
    const duration = (endTime.getTime() - currentTrip.startTime.getTime()) / (1000 * 60)
    // Usar el precio por minuto y factor de carbono del vehículo actual si está disponible
    const currentVehicle = stations.flatMap((s) => s.vehicles).find((v) => v.id === currentTrip.vehicleId)
    const pricePerMinute = currentVehicle?.pricePerMinute || 50 // Default a 50 si no se encuentra
    const carbonFactor = currentVehicle?.carbonFactor || 0.05 // Default a 0.05 si no se encuentra

    const cost = Number.parseFloat((duration * pricePerMinute).toFixed(2))
    const carbonSaved = Number.parseFloat((duration * carbonFactor).toFixed(2))
  ...
  const getOffPublicTransport = useCallback(
    async (stationName: string) => {
      ...
      // Calcular costo y carbono ahorrado para el viaje de transporte público
      // Esto es una simulación, ajusta los valores según tu lógica de negocio
      const duration = (Date.now() - currentPublicTransportTrip.startTime.getTime()) / (1000 * 60) // minutos
      const cost = Number.parseFloat((duration * 0.1).toFixed(2)) // Ejemplo: $0.1 por minuto
      const carbonSaved = Number.parseFloat((duration * 0.01).toFixed(2)) // Ejemplo: 0.01 kg CO2 por minuto
      const rating = 5 // Calificación predeterminada al finalizar automáticamente
    ...
    try {
      ...
      if (response.ok) {
        const newTrip: PublicTransportTrip = {
          id: data.trip.id.toString(),
          userId: Number.parseInt(data.trip.userId),
          //vehicleColectivoId: data.trip.vehicle_colectivo_id.toString(), // Asegúrate de que el backend devuelva esto
      ...
      publicTransportIntervalRef.current = setInterval(() => {
        setCurrentPublicTransportTrip((prevTrip) => {
          ...
          if (nextIndex >= plannedDestinationIndex) {
            ...
            getOffPublicTransport(prevTrip.plannedDestination)
